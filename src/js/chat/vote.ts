...
import { getPlayerList, initVote, setLiveOrDiePlayer, setPlayerList } from '../lib/store';
...
function addVote(targetId: string){
  ...
  if(totalVote === playerCount) {
    ...
    // 최다 득표수 계산
    const maxVote = Math.max(...Object.values(playerList).map(player => player.vote));
    
    // 과반수 이상 득표 여부 확인
    if(maxVote >= playerCount/2){
      // 최다 득표자 목록 추출
      const maxVotePlayers = Object.values(playerList).filter(player => player.vote === maxVote);
