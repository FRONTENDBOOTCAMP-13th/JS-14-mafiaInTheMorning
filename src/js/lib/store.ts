...
/**
 * 게임에서 죽지 않은 플레이어들의 수를 반환합니다.
 * @returns 현재 생존중인 플레이어 수
 */
export function getLivePlayerCount() {
    return Object.values(playerList).filter(player => !player.killed).length;
}

/**
 * 게임에서 죽음을 맞이한 플레이어의 killed 값을 저장합니다.
 * @param playerId 죽임을 당한 플레이어
 */
